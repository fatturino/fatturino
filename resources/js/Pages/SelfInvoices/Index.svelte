...
- Codice destinatario o PEC`
    ...
    async function savePayment() {
        ...
        const url = isEdit
            ? `/self-invoices/${paymentInvoice.id}/payments/${editingPaymentId}`
            : `/self-invoices/${paymentInvoice.id}/payments`
    ...
    async function deletePayment(payment) {
        const response = await fetch(`/self-invoices/${paymentInvoice.id}/payments/${payment.id}`, {
            method: 'DELETE',
            headers: {
                'Accept': 'application/json',
                'X-XSRF-TOKEN': csrfToken(),
