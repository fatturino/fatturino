<script>
    ...
    async function validateXml(invoice) {
        ...
        onConfirmAction = async () => {
            await postAction(`/sell-invoices/${invoice.id}/validate-xml`, 'XML validato.')
        }
        confirmOpen = true
    }
...
- Codice destinatario o PEC`
        ...
        onConfirmAction = async () => {
            await postAction(`/sell-invoices/${invoice.id}/send-sdi`, 'Fattura inviata allo SDI.')
        }
        confirmOpen = true
    }

    async function sendEmail(invoice) {
        await postAction(`/sell-invoices/${invoice.id}/send-email`, 'Email inviata.')
    }
    ...
    async function savePayment() {
        ...
        const url = isEdit
            ? `/sell-invoices/${paymentInvoice.id}/payments/${editingPaymentId}`
            : `/sell-invoices/${paymentInvoice.id}/payments`
    ...
    async function deletePayment(payment) {
        const response = await fetch(`/sell-invoices/${paymentInvoice.id}/payments/${payment.id}`, {
            method: 'DELETE',
            headers: {
                'Accept': 'application/json',
                'X-XSRF-TOKEN': csrfToken(),
        ...
        <section class="card-brand overflow-hidden hidden md:block">
            <table class="w-full text-sm">
                ...
                <tbody>
                            ...
                            {#snippet children({ triggerProps })}
                                    <tr {...triggerProps} class="border-b border-border-light hover:bg-surface-muted/70 transition-colors cursor-context-menu">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <a
                                                href={`/sell-invoices/${invoice.id}/edit`}
                    ...
                    {:else}
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-brand-secondary/60">
                                {hasActiveFilters() ? 'Nessuna fattura trovata con questi filtri.' : 'Nessuna fattura ancora emessa.'}
                            </td>
                        </tr>
                    {/each}
                </tbody>
