...
<div class="page">
    ...
    <table class="meta-table">
        @php
            $formatDateForPdf = static function ($value): string {
                if (empty($value)) {
                    return '';
                }
        ...
        <tr>
            ...
            <td class="meta-cell">
                ...
                <div class="meta-value">{{ $formatDateForPdf($invoice->date) }}</div>
            </td>
            @if ($invoice->due_date)
                <td class="meta-cell">
                    <div class="meta-label">{{ __('app.pdf.due_date') }}</div>
                    <div class="meta-value">{{ $formatDateForPdf($invoice->due_date) }}</div>
                </td>
            @endif
                        ...
                        <td>
                            ...
                            @if ($invoice->due_date)
                                <div style="font-size:8px; font-weight:normal; margin-top:3px; opacity:0.7;">
                                    {{ __('app.pdf.due_date') }}: {{ $formatDateForPdf($invoice->due_date) }}
                                </div>
                            @endif
                        </td>
