...
<article class="dashboard-module p-2">
    ...
    <div class="mt-5 divide-y divide-border-light">
        @forelse($items as $item)
            <x-app-link :href="$item['href']" class="dashboard-list-link -mx-2 flex items-center gap-3 border-0 px-2"><span class="min-w-0 flex-1"><span class="block text-sm font-medium text-content">{{ $item['title'] }}</span><span class="mt-0.5 block text-xs leading-5 text-content-muted">{{ $item['detail'] }}</span></span><span class="shrink-0 text-right"><span @class(['block text-sm font-semibold tabular-nums', 'text-danger' => $item['tone'] === 'danger', 'text-content' => $item['tone'] !== 'danger'])>{{ $item['value'] }}</span><span class="mt-0.5 block text-xs font-medium text-primary">{{ $item['action'] }}</span></span></x-app-link>
        @empty
