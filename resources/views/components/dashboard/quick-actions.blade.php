...
<x-card class="h-full">
    ...
    <div class="grid grid-cols-2 gap-2">
        {{-- New invoice (current year) or New credit note (past years) --}}
        @if($isCurrentYear)
            <x-app-link :href="route('sell-invoices.create')"
        ...
        @else
            <x-app-link href="/credit-notes/create"
               class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-primary/5 hover:bg-primary/10 border border-primary/20 transition-colors text-center">
                <span class="w-9 h-9 rounded-lg bg-primary flex items-center justify-center">
                    <x-icon name="o-receipt-refund" class="w-5 h-5 text-white" />
                </span>
                <span class="text-xs font-medium text-primary">{{ __('app.dashboard.quick_credit_note') }}</span>
            </x-app-link>
        @endif

        {{-- Self invoice --}}
        <x-app-link href="/self-invoices/create"
        ...
        {{-- New contact --}}
        <x-app-link href="/contacts/create"
