...
@else
    <div wire:key="line-{{ $line['key'] }}" class="space-y-2 rounded-md border border-border-light bg-surface-muted p-3">
        ...
        @if($line['details_enabled'] ?? false)
            <div @class(['grid gap-3 rounded-md border border-border-light bg-white p-3', 'sm:grid-cols-2' => ! $hasDiscount, 'sm:grid-cols-3' => $hasDiscount])>
                ...
                <label class="text-xs text-content-muted">Unità di misura<input wire:model.blur="lines.{{ $index }}.unit_of_measure" @disabled($readOnly) placeholder="es. ore" class="mt-1 h-10 w-full rounded-md border border-border bg-white px-2 text-sm"></label>
                @if($hasDiscount)
                    <fieldset class="min-w-0">
                        <div class="flex min-h-5 items-center justify-between gap-2">
                            <legend class="text-xs text-content-muted">Sconto</legend>
                            <div class="inline-flex items-center gap-2 text-xs font-medium text-content-muted {{ $readOnly ? 'opacity-60' : '' }}">
                                <span>Applica</span>
                                <button type="button" role="switch" wire:click="toggleLineDiscount({{ $index }})" @disabled($readOnly) aria-checked="{{ ($line['discount_enabled'] ?? false) ? 'true' : 'false' }}" aria-label="Applica sconto alla riga" @class(['relative h-6 w-10 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20 focus:ring-offset-2 disabled:cursor-not-allowed', 'bg-primary' => $line['discount_enabled'] ?? false, 'bg-border-strong' => ! ($line['discount_enabled'] ?? false)])><span @class(['absolute left-1 top-1 size-4 rounded-full bg-white transition-transform', 'translate-x-4' => $line['discount_enabled'] ?? false])></span></button>
                            </div>
                        </div>
                        <input id="sales-line-{{ $line['key'] }}-discount" wire:model.live.debounce.250ms="lines.{{ $index }}.discount_percent" @disabled($readOnly || ! ($line['discount_enabled'] ?? false)) type="number" min="0" max="100" step="0.01" inputmode="decimal" aria-label="Percentuale sconto" class="mt-1 h-10 w-full rounded-md border border-border bg-white px-2 text-sm disabled:cursor-not-allowed disabled:bg-surface-muted disabled:text-content-muted">
                        @error("lines.$index.discount_percent")<span class="mt-1 block text-xs text-danger">{{ $message }}</span>@enderror
                    </fieldset>
                @endif
            </div>
        @endif
