...
<div
    x-data="{
        ...
        toggle() {
            ...
            if (this.open) {
                this.$nextTick(() => this.$refs.firstMenuItem?.focus());
            }
        },
...
>
    <button
        x-ref="trigger"
        type="button"
        class="inline-flex max-w-64 items-center gap-2.5 rounded-lg px-2 py-1.5 text-left text-sm text-content transition hover:bg-surface-muted focus:outline-none focus:ring-2 focus:ring-primary/20 max-sm:size-11 max-sm:justify-center max-sm:p-0"
    ...
    >
        <div class="divide-y divide-border">
            <div class="space-y-1 p-2.5">
                <x-app-link x-ref="firstMenuItem" :href="route('settings.company')" role="menuitem" class="group flex min-h-11 items-center gap-2 rounded-lg border border-transparent px-2.5 py-2 text-sm font-medium text-content-muted transition hover:bg-primary-subtle hover:text-primary">
            ...
            <div class="space-y-1 p-2.5">
                <form method="POST" action="{{ route('logout') }}" data-posthog-logout>
                    @csrf
                    <button type="submit" role="menuitem" class="group flex min-h-11 w-full items-center gap-2 rounded-lg border border-transparent px-2.5 py-2 text-left text-sm font-medium text-danger transition hover:bg-danger/5">
