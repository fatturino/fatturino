    ...
    >
        <div class="divide-y divide-border">
            <div class="space-y-1 p-2.5">
                <x-app-link x-ref="firstMenuItem" :href="route('settings.company')" class="group flex min-h-11 items-center gap-2 rounded-lg border border-transparent px-2.5 py-2 text-sm font-medium text-content-muted transition hover:bg-primary-subtle hover:text-primary">
            ...
            <div class="space-y-1 p-2.5">
                <form method="POST" action="{{ route('logout') }}" data-posthog-logout>
                    @csrf
                    <button type="submit" class="group flex min-h-11 w-full items-center gap-2 rounded-lg border border-transparent px-2.5 py-2 text-left text-sm font-medium text-danger transition hover:bg-danger/5">
