...
<aside class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col border-r border-indigo/40 bg-ink text-white transition-transform duration-300 lg:translate-x-0" :class="{ 'translate-x-0': sidebarOpen }" aria-label="Navigazione principale">
    ...
    <nav class="flex-1 overflow-y-auto p-4">
        <x-app-link :href="route('dashboard')" class="mb-4 block rounded-md px-3 py-2 text-sm font-semibold {{ request()->routeIs('dashboard') ? 'bg-teal' : 'text-white/80 hover:bg-white/10' }}">Dashboard</x-app-link>
        @foreach ($sections as $label => $items)
            ...
            @foreach ($items as [$item, $route])
                <x-app-link :href="route($route)" class="block rounded-md px-3 py-2 text-sm font-medium {{ request()->routeIs($route) ? 'bg-white/15 text-white' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">{{ $item }}</x-app-link>
            @endforeach
        @endforeach
    </nav>
</aside>
