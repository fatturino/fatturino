...
<section class="space-y-6">
    ...
    <article class="rounded-xl border border-border-light bg-white p-5 shadow-[var(--shadow-card)]"><h2 class="font-bold">Webhook</h2><p class="mt-2 text-sm text-content-muted">Callback: <code>{{ $webhookCallbackUrl }}</code></p><p class="mt-2 text-sm {{ $hasWebhookSecret ? 'text-success' : 'text-content-muted' }}">{{ $hasWebhookSecret ? 'Webhook configurato.' : 'Webhook non configurato.' }}</p>@if($sandbox)<div class="mt-4 border-t pt-4"><p class="text-sm font-semibold">Simulazione sandbox</p><div class="mt-3 flex flex-wrap gap-2"><select wire:model="simulationType" class="rounded-md border border-border px-3 py-2 text-sm"><option value="supplier-invoice">Supplier invoice</option><option value="customer-notification">Customer notification</option><option value="customer-invoice">Customer invoice</option></select><input wire:model="invoiceUuid" class="rounded-md border border-border px-3 py-2 text-sm" placeholder="Invoice UUID (opzionale)"><button wire:click="simulateWebhook" wire:loading.attr="disabled" wire:target="simulateWebhook" type="button" class="rounded-md border border-border px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-60"><span wire:loading.remove wire:target="simulateWebhook">Invia simulazione</span><span wire:loading wire:target="simulateWebhook">Invio...</span></button></div>@error('simulationType')<p class="mt-2 text-sm text-danger">{{ $message }}</p>@enderror @error('invoiceUuid')<p class="mt-2 text-sm text-danger">{{ $message }}</p>@enderror</div>@endif</article>
    <article class="rounded-xl border border-border-light bg-white p-5 shadow-[var(--shadow-card)]"><h2 class="font-bold">Conservazione digitale</h2>@if($conservationAcknowledged)<p class="mt-2 text-sm text-success">Obbligo di conservazione preso in carico.</p>@else<p class="mt-2 text-sm text-content-muted">Le fatture elettroniche devono essere conservate per dieci anni.</p><div class="mt-4 flex gap-2"><a class="rounded-md border border-border px-4 py-2 text-sm font-semibold" target="_blank" rel="noopener noreferrer" href="https://ivaservizi.agenziaentrate.gov.it">Apri Agenzia Entrate</a><button wire:click="acknowledgeConservation" wire:loading.attr="disabled" wire:target="acknowledgeConservation" type="button" class="rounded-md bg-primary px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60"><span wire:loading.remove wire:target="acknowledgeConservation">Ho preso visione</span><span wire:loading wire:target="acknowledgeConservation">Aggiornamento...</span></button></div>@endif</article>
</section>
