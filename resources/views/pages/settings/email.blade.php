...
new #[Layout('layouts::app')] class extends Component {
    ...
    public function mount(EmailSettings $settings): void { foreach (array_diff(array_keys($this->rules()), $this->secretFields()) as $field) $this->{$field} = $settings->{$field} ?? (is_bool($this->{$field}) ? false : ''); }
    public function save(EmailSettings $settings): void { $this->ensureAllowed(); $validated = $this->validate(); $secrets = collect($this->secretFields())->mapWithKeys(fn ($field) => [$field => $validated[$field] ?? null]); foreach ($this->secretFields() as $field) unset($validated[$field]); $settings->fill($validated); foreach ($secrets as $field => $value) if (filled($value)) $settings->{$field} = $value; $settings->save(); session()->flash('success', 'Impostazioni email salvate.'); }
    public function testConnection(EmailSettings $settings): void
    {
        try {
            ...
            $temporary->fill($data);
            foreach ($this->secretFields() as $field) if (! filled($data[$field] ?? null)) $temporary->{$field} = $settings->{$field};
        ...
        } catch (\Throwable $exception) { Log::warning('Email connection test failed.', ['exception' => $exception]); $this->addError('smtp', 'Impossibile verificare la connessione email. Controlla i dati configurati.'); }
    }
    ...
    private function secretFields(): array { return ['smtp_password', 'scaleway_tem_secret_key']; }
