...
new #[Layout('layouts::app')] class extends Component {
    ...
    public function mount(EmailSettings $settings): void { foreach (array_keys($this->rules()) as $field) $this->{$field} = $settings->{$field} ?? (is_bool($this->{$field}) ? false : ''); }
    public function save(EmailSettings $settings): void { $this->ensureAllowed(); $settings->fill($this->validate()); $settings->save(); session()->flash('success', 'Impostazioni email salvate.'); }
    public function testConnection(EmailSettings $settings): void
    {
        ...
        } catch (\Throwable $exception) { $this->addError('smtp', $exception->getMessage()); }
    }
    ...
    public function smtpManagedByEnv(): bool { return config('email.managed_by_env', false); }
    private function ensureAllowed(): void { abort_unless(app(EnvironmentCapabilities::class)->can('edit-email-settings'), 403, 'Operazione non consentita in questa modalità.'); }
}; ?>
...
<div class="grid gap-6 lg:grid-cols-2"><article class="rounded-xl border border-border-light bg-white p-5 shadow-[var(--shadow-card)]"><h2 class="font-bold">Provider invio email</h2>@if($this->smtpManagedByEnv())<p class="mt-4 text-sm text-content-muted">Configurato tramite variabili d'ambiente.</p>@else<div class="mt-4 space-y-4"><label class="block text-sm font-semibold">Provider<select wire:model.live="mail_provider" class="mt-2 block w-full rounded-md border border-border px-3 py-2"><option value="smtp">SMTP</option><option value="scaleway_tem">Scaleway TEM</option></select></label>@if($mail_provider === 'scaleway_tem')<x-settings.input wire:model="scaleway_tem_region" label="Regione"/><x-settings.input wire:model="scaleway_tem_project_id" label="Project ID"/><x-settings.input wire:model="scaleway_tem_secret_key" type="password" label="Secret key"/>@else<x-settings.input wire:model="smtp_host" label="Host"/><div class="grid grid-cols-2 gap-4"><x-settings.input wire:model="smtp_port" label="Porta"/><label class="block text-sm font-semibold">Crittografia<select wire:model="smtp_encryption" class="mt-2 block w-full rounded-md border border-border px-3 py-2"><option value="">Nessuna</option><option value="tls">TLS</option><option value="ssl">SSL</option></select></label></div><x-settings.input wire:model="smtp_username" label="Username"/><x-settings.input wire:model="smtp_password" type="password" label="Password"/>@endif</div>@endif</article>
