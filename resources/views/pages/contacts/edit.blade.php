...
<x-slot:header><div><p class="text-xs font-bold uppercase tracking-[.12em] text-content-muted">Anagrafiche</p><h1 class="text-lg font-bold text-content">Modifica contatto</h1></div></x-slot:header>

<section class="mx-auto max-w-4xl"><form wire:submit="save" class="space-y-6"><article class="rounded-xl border border-border-light bg-white p-5 shadow-[var(--shadow-card)]"><div class="grid gap-4 sm:grid-cols-2"><x-settings.input wire:model="name" label="Nome *"/><label class="block text-sm font-semibold">Nazione *<select wire:model.live="country" class="mt-2 block w-full rounded-md border border-border px-3 py-2">@foreach($this->countries() as $code => $label)<option value="{{ $code }}">{{ $label }}</option>@endforeach</select>@error('country')<span class="mt-1 block text-xs text-danger">{{ $message }}</span>@enderror</label><x-settings.input wire:model="vat_number" label="Partita IVA"/><x-settings.input wire:model="tax_code" label="Codice fiscale"/><x-settings.input wire:model="email" type="email" label="Email"/><x-settings.input wire:model="pec" type="email" label="PEC"/><x-settings.input wire:model="sdi_code" label="Codice SDI"/></div></article><article class="rounded-xl border border-border-light bg-white p-5 shadow-[var(--shadow-card)]"><h2 class="font-bold">Indirizzo</h2><div class="mt-4 grid gap-4 sm:grid-cols-2"><div class="sm:col-span-2"><x-settings.input wire:model="address" label="Indirizzo"/></div><x-settings.input wire:model="postal_code" label="CAP"/><x-settings.input wire:model="city" label="Città"/><x-settings.input wire:model="province" label="Provincia" maxlength="2"/></div></article><div class="flex gap-3"><button class="rounded-md bg-primary px-5 py-2.5 text-sm font-bold text-white" type="submit">Salva contatto</button><x-app-link :href="route('contacts.index')" class="rounded-md border border-border px-5 py-2.5 text-sm font-semibold">Annulla</x-app-link></div></form></section>
