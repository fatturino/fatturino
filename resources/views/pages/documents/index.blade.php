...
@php($definition = $this->definition())
<section class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-[.12em] text-content-muted">Anno fiscale {{ $fiscalYear }}</p><p class="mt-1 text-sm text-content-muted">Gestisci e monitora i tuoi documenti.</p></div>@if($definition['create'])<a href="/{{ $definition['base'] }}/create" class="rounded-md bg-primary px-4 py-2 text-sm font-bold text-white">{{ $definition['create'] }}</a>@endif</div>
