...
new #[Layout('layouts::app')] class extends Component {
    ...
    {
        $rules = ['contact_id' => 'required|exists:contacts,id', 'date' => 'required|date', 'due_date' => 'nullable|date', 'document_type' => 'required|in:TD17,TD18,TD19,TD28,TD29', 'related_invoice_number' => 'required|string|max:20', 'related_invoice_date' => 'required|date', 'notes' => 'nullable|string', 'lines' => 'required|array|min:1', 'lines.*.description' => 'required|string', 'lines.*.quantity' => 'required|numeric|min:0.01', 'lines.*.unit_of_measure' => 'nullable|string', 'lines.*.unit_price' => 'required|numeric|min:0', 'lines.*.vat_rate' => 'required|string'];
    ...
    <form wire:submit="save" x-data="{ dirty: false, tab: @entangle('tab'), tabs: ['data', 'notes' @if($invoice), 'history' @endif], selectTab(nextTab) { this.tab = nextTab; this.$nextTick(() => document.getElementById('self-invoice-tab-' + nextTab)?.focus()); }, moveTab(step) { this.selectTab(this.tabs[(this.tabs.indexOf(this.tab) + step + this.tabs.length) % this.tabs.length]); } }" @beforeunload.window="if (dirty) { $event.preventDefault(); $event.returnValue = ''; }" @input="dirty = true" @change="dirty = true" @sales-line-added.window="$nextTick(() => document.getElementById('sales-line-' + $event.detail.key + '-description')?.focus())" @sales-line-removed.window="$nextTick(() => document.getElementById('sales-line-' + $event.detail.key + '-description')?.focus())" class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <article class="rounded-xl border border-border bg-white p-5 sm:p-6">
            <x-documents.invoice-form.data-section variant="sales-editor">
                ...
                <section id="self-invoice-panel-data" role="tabpanel" aria-labelledby="self-invoice-tab-data" class="space-y-5 pt-5" x-show="tab === 'data'"><x-documents.invoice-form.data-fields><label class="text-sm font-semibold">Fornitore *<x-select wire:model="contact_id" :disabled="$this->readOnly" :options="$contactOptions" searchable searchPlaceholder="Cerca per nome o P.IVA" />@error('contact_id')<span class="text-xs text-danger">{{ $message }}</span>@enderror</label><div class="text-sm font-semibold">Numero<div class="mt-1 h-11 rounded-md border border-border-light bg-surface-muted px-3 py-3 text-sm font-normal">{{ $numberPreview ?? 'Configura il sezionale predefinito' }}</div></div><label class="text-sm font-semibold">Tipo documento *<x-select wire:model="document_type" :disabled="$this->readOnly" :options="['TD17' => 'TD17 - Acquisto servizi dall’estero', 'TD18' => 'TD18 - Acquisto beni intracomunitari', 'TD19' => 'TD19 - Acquisto beni ex art.17', 'TD28' => 'TD28 - San Marino con IVA', 'TD29' => 'TD29 - Omessa/irregolare fatturazione']" /></label><x-date-picker label="Data" wire:model.live="date" :required="true" :disabled="$this->readOnly" /><x-date-picker label="Scadenza" wire:model="due_date" :disabled="$this->readOnly" /></x-documents.invoice-form.data-fields><section class="rounded-lg border border-primary/20 bg-primary/5 p-4 sm:p-5" aria-labelledby="related-invoice-heading"><div class="flex flex-wrap items-start justify-between gap-2"><div><h3 id="related-invoice-heading" class="text-sm font-semibold text-content">Riferimenti della fattura collegata</h3><p class="mt-1 text-sm text-content-muted">Indica numero e data della fattura del fornitore a cui si riferisce l’autofattura.</p></div><span class="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">Entrambi obbligatori</span></div><div class="mt-4 grid gap-4 sm:grid-cols-2"><label class="text-sm font-semibold text-content">Numero fattura collegata <span aria-hidden="true">*</span><span class="sr-only"> obbligatorio</span><input wire:model="related_invoice_number" @disabled($this->readOnly) aria-describedby="related-invoice-number-error" class="mt-1 h-11 w-full rounded-lg border border-border-strong bg-white px-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"></label><x-date-picker label="Data fattura collegata" wire:model="related_invoice_date" :required="true" :disabled="$this->readOnly" />@error('related_invoice_number')<p id="related-invoice-number-error" class="-mt-2 text-xs text-danger sm:col-span-2">{{ $message }}</p>@enderror</div></section></section>
