...
class PurchaseInvoice extends FiscalDocument
{
    use HasFactory;
    use HasParent;
    use HasPayments;
    ...
    /**
     * Net amount due to pay (gross - withholding tax). All amounts in cents.
     */
    ...
    public static function createOrUpdateFromSdiData(array $invoiceData, Contact $contact): ?self
    {
        ...
        if ($documentNumber) {
            ...
            if ($selfInvoice) {
                $selfInvoice->update([
                    'sdi_uuid' => $selfInvoice->sdi_uuid ?: ($invoiceData['uuid'] ?? null),
