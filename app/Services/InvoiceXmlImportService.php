...
class InvoiceXmlImportService
{
    ...
    public function importXml(string $xmlContent, ?int $sequenceId, string $category = 'sales', array $sdiContext = []): void
    {
        ...
        try {
            ...
            $businessFingerprint = $this->businessFingerprintService->buildFromXml($xmlContent);
            $header = $xml->FatturaElettronicaHeader;
            $body = $xml->FatturaElettronicaBody;

            DB::transaction(function () use ($header, $body, $sequenceId, $category, $xmlContent, $sdiContext, $businessFingerprint) {
                ...
                [$modelClass, $status, $sdiStatus] = match ($category) {
                    ...
                    default => [FiscalDocument::class, InvoiceStatus::Sent, SdiStatus::Delivered],
                };
                ...
                $alreadyExists = $modelInstance->newQuery()
                    ->where(function ($query) use ($number, $contact, $year, $businessFingerprint) {
                        $query->where(function ($sub) use ($number, $contact, $year) {
                            $sub->where('number', $number)
                                ->where('contact_id', $contact->id)
                                ->where('fiscal_year', $year);
                        });

                        $query->orWhere('business_fingerprint', $businessFingerprint);
                    })
                    ->first();

                if ($alreadyExists) {
                    $this->applyMissingSdiContext($alreadyExists, $sdiContext);
                    $this->stats['skipped']++;
                ...
                $invoice = $this->createInvoice($body, $contact, $sequenceId, $status, $sdiStatus, $modelClass, $sdiContext, $businessFingerprint);
    ...
    protected function createInvoice(
        ...
        string $modelClass = FiscalDocument::class,
        array $sdiContext = [],
        ?string $businessFingerprint = null,
    ): Model {
        ...
        if (($sdiContext['sdi_file_id'] ?? null) !== null && $sdiContext['sdi_file_id'] !== '') {
            $attributes['sdi_file_id'] = $sdiContext['sdi_file_id'];
        }

        if (($sdiContext['sdi_filename'] ?? null) !== null && $sdiContext['sdi_filename'] !== '') {
            $attributes['sdi_filename'] = $sdiContext['sdi_filename'];
        }

        if (! empty($sdiContext['sdi_received_at'])) {
            $attributes['sdi_received_at'] = $sdiContext['sdi_received_at'];
        }

        if (! empty($sdiContext['sdi_payload'])) {
            $attributes['sdi_payload'] = $sdiContext['sdi_payload'];
        }
    ...
    /**
     * Create FiscalDocumentLine records from XML line items.
     */
    protected function createFiscalDocumentLines(\SimpleXMLElement $datiBeniServizi, Model $invoice): void
    {
    ...
    /**
     * Persist SDI metadata only when the target invoice is still missing it.
     */
    protected function applyMissingSdiContext(Model $invoice, array $sdiContext): void
    {
        $updates = [];

        if (($sdiContext['sdi_uuid'] ?? null) && empty($invoice->sdi_uuid)) {
            $updates['sdi_uuid'] = $sdiContext['sdi_uuid'];
        }

        if (($sdiContext['sdi_file_id'] ?? null) !== null && $sdiContext['sdi_file_id'] !== '' && empty($invoice->sdi_file_id)) {
            $updates['sdi_file_id'] = $sdiContext['sdi_file_id'];
        }

        if (($sdiContext['sdi_filename'] ?? null) && empty($invoice->sdi_filename)) {
            $updates['sdi_filename'] = $sdiContext['sdi_filename'];
        }

        if (! empty($sdiContext['sdi_received_at']) && empty($invoice->sdi_received_at)) {
            $updates['sdi_received_at'] = $sdiContext['sdi_received_at'];
        }

        if (! empty($sdiContext['sdi_payload']) && empty($invoice->sdi_payload)) {
            $updates['sdi_payload'] = $sdiContext['sdi_payload'];
        }
