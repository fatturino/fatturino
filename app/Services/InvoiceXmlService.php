...
class InvoiceXmlService
{
     ...
     */
    public function generateFileName(FiscalDocument $invoice): string
    {
        return $this->buildSdiFilename($this->companySettings, $invoice->id);
    }

    public function generate(FiscalDocument $invoice): string
    {
        ...
        if ($this->companySettings->company_fiscal_regime !== 'RF19' && $invoice->withholding_tax_enabled && $invoice->withholding_tax_amount) {
        ...
        if ($invoice->payment_method) {
            ...
            $paymentMethodValue = $invoice->payment_method instanceof \BackedEnum
                ? $invoice->payment_method->value
                : $invoice->payment_method;
            $paymentDetails->setMethod($paymentMethodValue);
